...
export default function Historique({ orders, refresh, clientName, setOrders }) {
    ...
    const deleteCommande = async (commandeId, all) => {
        try {
            const answer = await axios.post('http://localhost:3700/orders/delete',
                {
                    clientName,
                    commandeId,
                    all
    ...
    return (
        <>
            <div className="content" style={{ height: 500, overflow: 'scroll' }}>
                <div className="px-3 w-100 bg-secondary d-flex justify-content-between align-items-center rounded text-light" style={{ height: 50 }}>
                    <strong>
                        nom
                    </strong>
                    <strong>
                ...
                </div>
                {
                    orders
                        ? orders.map((order, index) => (
                            <>
                                <div key={index} className=" bg-light d-flex justify-content-between px-3 align-items-center  mt-2" style={{ height: 80, borderRadius: '20px', width: '100%' }}>
                                    ...
                                    <h4 onClick={() => { deleteCommande(order._id, false) }} style={{ cursor: 'pointer' }}><i className="bi bi-x-lg"></i></h4>
                                </div>
                                <div className='bg-light text-center text-secondary rounded' style={{ width: '14%' }}>
                                    {order.date}
                                </div>
